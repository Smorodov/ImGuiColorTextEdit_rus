		...
		while (lineNo <= lineMax)
		{
			...
			if (errorIt != mErrorMarkers.end())
			{
				...
				if (ImGui::IsMouseHoveringRect(lineStartScreenPos, end))
				{
					...
					ImGui::Text(u8"Îøèáêà â ñòðîêå %d:", errorIt->first);
					ImGui::PopStyleColor();
					ImGui::Separator();
			...
			if (runningIt != mRunningMarkers.end())
			{
				...
				if (ImGui::IsMouseHoveringRect(lineStartScreenPos, end))
				{
					...
					ImGui::Text(u8"Ïåðåìåííûå ñîñòîÿíèÿ. Ñòðîêà %d:", runningIt->first);
					ImGui::PopStyleColor();
					ImGui::Separator();
					ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 0.2f, 1.0f));
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::HLSL()
{
	...
	if (!inited)
	{
		...
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::wstring, PaletteIndex>(L"[ \\t]*#[ \\t]*[a-zA-ZÀ-ßà-ÿ_]+", PaletteIndex::Preprocessor));
		...
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::wstring, PaletteIndex>(L"[a-zA-ZÀ-ßà-ÿ_][a-zA-ZÀ-ßà-ÿ0-9_]*", PaletteIndex::Identifier));
